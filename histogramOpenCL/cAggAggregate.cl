...
void __kernel dimensionAggregate(__global cAgg *c_agg, __global cAgg *c_agg_gen, __global int *cubeDim, __global int *cubeDim_gen, __global int *dimToAggregate, __global int *kernelN)
{
    int deviceId = get_global_id(0);
    int totalKernelNumber = kernelN[0];
    int totalCellsInGenCube = cubeDim[0] * cubeDim[1] * cubeDim[2];
    if(dimToAggregate[0] == X_AGGREGATE) totalCellsInGenCube /= cubeDim[0];
    else if(dimToAggregate[0] == Y_AGGREGATE) totalCellsInGenCube /= cubeDim[1];
    else if(dimToAggregate[0] == Z_AGGREGATE) totalCellsInGenCube /= cubeDim[2];

    for(int axisInGenCube = deviceId * totalCellsInGenCube / totalKernelNumber; axisInGenCube < (deviceId + 1) * totalCellsInGenCube / totalKernelNumber; axisInGenCube++)
    {
        ...
        if(dimToAggregate[0] == X_AGGREGATE && cubeDim[0] == 1 ||
           dimToAggregate[0] == Y_AGGREGATE && cubeDim[1] == 1 ||
           dimToAggregate[0] == Z_AGGREGATE && cubeDim[2] == 1) return;
